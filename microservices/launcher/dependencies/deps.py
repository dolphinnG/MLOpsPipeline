...
def get_torchx_launcher():
    ...
    return TorchxLauncher(
        namespace=settings.VOLCANO_NAMESPACE, queue=settings.VOLCANO_QUEUE, s3_service=get_s3_service()
    )
...
def get_redis_service():
    return RedisService(host=settings.REDIS_HOST, port=settings.REDIS_PORT, db=settings.REDIS_DB)
